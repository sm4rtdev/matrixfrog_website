...
const MATRIX_CACHE_TTL = 15000;  // 15 seconds for MATRIX (more frequently updated)
...
export async function GET(request: Request) {
  ...
  const isMatrixToken = token?.toLowerCase() === '0x2044682dad187456af1eee1b4e02bbf0a9abc919';
  ...
  try {
    ...
    if (isNative) {
      ...
      const tokenData = data.find(
        (item: TokenData) => item.token && item.token.address && 
      ...
      if (tokenData) {
        ...
        if (token?.toLowerCase() === '0xFa6882945A345d6b74e87A8d810d3B9dD3043162') { // USDT on PEPU Chain
          decimals = 6;
        }
      } else {
