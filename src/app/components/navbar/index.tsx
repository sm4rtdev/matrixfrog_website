...
//   // Check network
//   useEffect(() => {
//     if (chainId && chainId !== 3409) {
//       alert("Please switch to Pepe Unchained Mainnet (Chain ID: 3409)");
//     }
//   }, [chainId]);
...
//     console.log("Decimals Error:", decimalsError);
...
//     try {
//       // Attempt to switch to Pepe Unchained Mainnet
//       await window.ethereum.request({
//         method: "wallet_switchEthereumChain",
//         params: [{ chainId: "0xd51" }], // 3409 in hex
//       });
...
//               },
//               rpcUrls: [
//                 "https://rpc-pepe-unchained-gupg0lo9wf.t.conduit.xyz/ca6fby1he3c2vegepqgxcgv8ephpyhwpz",
//               ],
//               blockExplorerUrls: [
...
export default function Navbar() {
  const [isMobile, setIsMobile] = useState(false);
  const [menuOpen, setMenuOpen] = useState(false);
  const [walletConnected, setWalletConnected] = useState(false); // Controls dropdown visibility
  const [tokenBalance, setTokenBalance] = useState<string>("0");
  const [tokenError, setTokenError] = useState(false);
  const { connect } = useConnect();
  const { isConnected, address } = useAccount();
  ...
  const connectMetaMask = () => {
    connect({ connector: injected() });
    setWalletConnected(false); // Close dropdown after selection
  };
  ...
  const handleDisconnect = () => {
    disconnect();
    setTokenBalance("0");
    setWalletConnected(false); // Ensure dropdown is closed
  };
