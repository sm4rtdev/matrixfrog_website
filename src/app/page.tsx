...
export default function Home() {
  ...
  const [pillFlashEffect, setPillFlashEffect] = useState<'none' | 'red' | 'blue'>('none');
  ...
  const [quoteTransition, setQuoteTransition] = useState<'idle' | 'fadeOut' | 'typing'>('idle');
  ...
  const bluePillQuotes = [
    ...
    "The Matrix is a system, Fren. That system is our enemy.",
  ...
  const handleRedPill = () => {
    ...
    setTimeout(() => {
      ...
      const randomQuote = redPillQuotes[Math.floor(Math.random() * redPillQuotes.length)];
  ...
  const handleBluePill = () => {
    ...
    setTimeout(() => {
      // Zufälliges Zitat auswählen
      const randomQuote = bluePillQuotes[Math.floor(Math.random() * bluePillQuotes.length)];
      setMatrixQuote(randomQuote);
      setDisplayedQuote("");
      setQuoteTransition('typing');
    }, 400); // Nach der FadeOut-Animation (300ms)
  ...
  }, [loading, typedText, currentTextIndex, textComplete, textToType, matrixQuote]);
  
  // Matrix rain effect for the main page after loading
  useEffect(() => {
    if (loading) return;
    ...
    // Mix of Matrix characters and frog-related symbols
    ...
    const draw = () => {
      ...
      // Matrix Regen Farbe - immer im Matrix-Grün
      ctx.fillStyle = 'rgba(0, 255, 65, 0.75)';
      ctx.font = `${fontSize}px monospace`;
      
      for (let i = 0; i < drops.length; i++) {
        // Reduced density of characters
        if (Math.random() > 0.4) { // 60% chance to draw a character
  ...
  return (
    <>
      ...
      <main className="flex min-h-screen flex-col items-center justify-between relative overflow-x-hidden bg-black">
        {/* Matrix background - fixed position for scrolling effect */}
        <canvas 
          ref={canvasRef} 
        ...
        {/* Loading overlay */}
        {loading && (
          <InitialPage onComplete={handleInitialComplete} />
        )}
          ...
          <div className="w-full z-20">
            ...
            <div className="min-h-[70vh] flex items-center justify-center px-4 pt-16">
              <div className="max-w-6xl w-full">
                  ...
                  <div className="w-full flex justify-center fade-in fade-in-delay-1">
                    <div 
                      className={`terminal-container bg-black bg-opacity-50 border border-matrix-green rounded-md p-4 scale-in-center ${
                        pillFlashEffect !== 'none' ? 'terminal-flash terminal-glitch' : ''
                      }`}
                    >
                      {/* Matrix Zitat (wenn vorhanden) oder Standard-Text */}
                      {matrixQuote ? (
                        <div className="text-matrix-green p-2 overflow-hidden">
                          <p className={`text-xl leading-relaxed ${
                            quoteTransition === 'fadeOut' ? 'text-fade-out' : 
                            quoteTransition === 'typing' ? 'text-fade-in' : ''
                          }`}>
                            {displayedQuote}
                            {quoteTransition === 'typing' && <span className="terminal-cursor-inline"></span>}
                          </p>
                        </div>
                      ) : (
                        <div className={quoteTransition === 'fadeOut' ? 'text-fade-out' : ''}>
                          {currentTextIndex === 0 && (
                            <p className="text-matrix-green text-xl terminal leading-relaxed">
                              {typedText}
                              {!textComplete && currentTextIndex === 0 && <span className="terminal-cursor-inline"></span>}
                            </p>
                          )}
                          ...
                          {(currentTextIndex === 1 || textComplete) && (
                            <p className="text-matrix-green text-base mt-4 opacity-80">
                              {currentTextIndex === 1 ? typedText : textToType[1]}
                              {!textComplete && currentTextIndex === 1 && <span className="terminal-cursor-inline"></span>}
                              {textComplete && <span className="terminal-cursor"></span>}
                            </p>
                          )}
                        </div>
                      )}
                ...
                <div className="w-full flex justify-center mb-2 relative fade-slide-up" style={{ height: '200px', animationDelay: '0.6s' }}>
                  <div className="pills-container">
                    {/* Red Pill */}
                    <div className="absolute left-pill-position pill-reveal pill-reveal-red" style={{ transform: 'translateY(10px)' }}>
                    ...
                    {/* Blue Pill */}
                    <div className="absolute right-pill-position pill-reveal pill-reveal-blue" style={{ transform: 'translateY(10px)' }}>
                      <div className="text-center mb-2">
                        <span className="glow-text-blue">CHOOSE BLUE PILL</span>
                      </div>
                      <div 
                        className="image-container cursor-pointer pill-hover"
            ...
            {/* Add Matrix Scroll Arrow Component */}
            <MatrixScrollArrow />
