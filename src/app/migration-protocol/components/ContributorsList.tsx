...
const ContributorsList: React.FC<ContributorsListProps> = ({
  ...
  useEffect(() => {
    console.log(
      "ContributorsList - MatrixFrog Contributors:",
      matrixContributors
    );
  }, [matrixContributors]);
  ...
  return (
    <div className="terminal-frame contributors-terminal mb-6">
      <div className="terminal-header">
        ...
        {/* Terminal Title */}
        <div className="terminal-title">TOP CONTRIBUTORS</div>
      </div>

      <div className="terminal-content p-4">
        {/* MatrixFrog Contributors */}
        <>
          {matrixLoading ? (
            ...
            <div className="text-center py-6 text-green-600 font-mono">
            ...
            <div className="contributors-list full-height-scroll custom-scrollbar">
              <div className="contributor-table">
                ...
                <div className="contributor-header sticky top-0 bg-black bg-opacity-95 z-10">
                  ...
                  <div className="contributor-cell head-cell text-right">
                    AMOUNT
                  </div>
                  <div className="contributor-cell head-cell text-right">
                    INFO
                ...
                {/* Contributor-Zeilen */}
                {safeMatrixContributors.map(
                  (contributor: Contributor, idx: number) => (
                    <div
                      key={`matrix-${idx}`}
                      className={`contributor-row ${
                        idx < 3 ? "top-contributor" : ""
                      }`}
                    >
                      ...
                      <div className="contributor-cell info-cell">
                        <span className="transfer-count">
                          {contributor.transferCount}×
                        </span>
                        <span className="last-date">
                          ({formatDate(contributor.lastTransfer)})
                        </span>
                      </div>
                    </div>
                  )
      ...
      <div className="terminal-content pt-0 px-4 pb-2">
        <div className="text-center text-xs text-green-700 mt-2">
          <span className="contributors-stat">
            {`${safeMatrixContributors.length} TOTAL MatrixFrog CONTRIBUTORS (L2)`}
          </span>
        </div>
      </div>
    </div>
